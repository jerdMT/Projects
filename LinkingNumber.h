...
{       std::cout<< "computing lkscan: "<< std::endl;
	...
	for (int i = 0; i<(400-n_vertices1); i+=50)
	{
	 ...
	 for ( int j=0; j<(400-n_vertices2); j+=50)
	 {
...
double center_o_mass(std::vector<double> chain1, std::vector<double> chain2)
{
	
	//subsystems can be treated as particles
	int count1 = 400; //treating point masses as 1 
	int count2 = 400; 
	int count_total = count1+ count2;
	
	int sum1 = std::accumulate(chain1.begin(), chain1.end(), 0);
	int sum2 = std::accumulate(chain2.begin(), chain2.end(), 0);
	
	double com1 = sum1/count1;
	double com2 = sum2/count2;
	
	double com_total = ((count1*com1) + (count2*com2))/ count_total;
