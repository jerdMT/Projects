...
{ 	
  ...
  int n_vertices1 = 50;  //For scanning function, segment length of 1st chain
  int n_vertices2 = 50; // segment length of second chain 
...
{  
 ...
 for (int i = 0; i<2381; i++) //how ever many lines done already
 {
     ...
     std::pair<int,int> presentPair(coord_i[i]-1,coord_j[i]-1);
     indices.push_back(presentPair); 
 }

 auto start = high_resolution_clock::now();
  for(int i=1; i<NumberofChainPairs; i++) 
  {
     //i is ID
